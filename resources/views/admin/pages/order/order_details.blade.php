                ...
                <div class="col-12">
                    ...
                    <div class="card">
                        <div class="card-header d-flex justify-content-between align-items-center">
                            ...
                            <div>
                                <a href="{{ route('admin.invoice.mail', $order->id) }}" class="btn btn-primary">Send Invoice via mail</a>
                                <a href="{{ route('admin.invoice.view', $order->id) }}" target="blank" class="btn btn-info">View Invoice</a>
                                <a href="{{ route('admin.invoice.generate', $order->id) }}" class="btn btn-primary">Download Invoice</a>
                                        ...
                                        <div class="card-body">
                                            <form action="{{ route('admin.orders.update',$order->id) }}" method="post">
                                                ...
                                                <select class="form-select" name="order_status" aria-label="Default select example">
                                                    ...
                                                    <option value="in progress" {{ $order->status == 'in progress' ? 'in progress' : '' }}>In Progress</option>
                                                    <option value="cancelled" {{ $order->status == 'cancelled' ? 'selected' : '' }}>Cancelled</option>
                                                    <option value="out of delivery" {{ $order->status == 'out of delivery' ? 'selected' : '' }}>Out of Delivery</option>
                                                </select>
                                                <button type="submit" class="btn btn-primary btn-sm mt-3"><b>Update</b></button>
                                            </form>
                                        </div>
                                      </div>
                                </div>
